...
export interface IAllowDropFn {
  (element: TreeNode, to: { parent: TreeNode, index: number }, $event?: Event): boolean;
}
...
export interface ITreeNode<T = unknown> {
   ...
   */
  doForAll<R>(fn: (node: ITreeNode) => R): void;
  /**
...
export interface ITreeModel {
   ...
   */
  setFocusedNode(node: ITreeNode): void;
  /**
   ...
   */
  getNodeByPath(path: IDType[], startNode?: ITreeNode): ITreeNode;
  /**
   ...
   */
  getNodeBy(predicate: (node: ITreeNode) => boolean, startNode?: ITreeNode): ITreeNode;
  /**
   * get tree state
   */
  getState(): ITreeState;
   ...
   */
  filterNodes(filter: string | ((node: ITreeNode) => boolean), autoShow?: boolean): void;
  /**
   ...
   */
  moveNode(node: ITreeNode, to: DragToSpot, from: DragFromSpot): void;
  /**
